...
if (en.type === 'boss') {
            ...
            if (en.state === 'entering') {
                ...
                // NERF: Slower charge speed (was 12, now 8.5)
                if (chargeDist > 10) { en.x += (chargeDx / chargeDist) * 8.5; en.y += (chargeDy / chargeDist) * 8.5; }
                
                if (dist < en.radius + player.radius && !player.isDashing) {
                    health -= 2; updateUI(); triggerShake(30); triggerHitStop(15);
                    spawnFloatingText(player.x, player.y - 40, "-2 ARMOR", "#ff003c", 30);
                    en.state = 'bullet_hell'; en.stateTimer = 120;
                    if (health <= 0) gameOver = true;
                }
                if (en.stateTimer <= 0) { en.state = 'bullet_hell'; en.stateTimer = 200; }
            }
        }
