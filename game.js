...
const player = { 
   ...
   color: selectedPlayerColor, // <--- Change this line!
...
function createEnemy(type, x, y) {
    ...
    if (type === 'standard') return { x, y, radius: 25, speed: 2.5 * diffMultiplier, color: '#ff003c', health: 5, maxHealth: 5, cooldown: 0, fireRate: 100 / diffMultiplier, ammo: 3, maxAmmo: 3, type: 'standard', flashTimer: 0, overclockTimer: 0, pierceTimer: 0 };
    if (type === 'sniper') return { x, y, radius: 20, speed: 1.2 * diffMultiplier, color: '#aa00ff', health: 3, maxHealth: 3, cooldown: 0, fireRate: 150 / diffMultiplier, ammo: 2, maxAmmo: 2, type: 'sniper', flashTimer: 0, overclockTimer: 0, pierceTimer: 0 };
    if (type === 'brute') return { x, y, radius: 35, speed: 3.5 * diffMultiplier, color: '#ff8800', health: 12, maxHealth: 12, cooldown: 0, fireRate: 9999, ammo: 0, maxAmmo: 0, type: 'brute', flashTimer: 0, overclockTimer: 0, pierceTimer: 0 };
    
    // THE BOSS
    if (type === 'boss') return { x, y, radius: 50, speed: 2.0, color: '#ff003c', health: 50 * diffMultiplier, maxHealth: 50 * diffMultiplier, cooldown: 0, type: 'boss', state: 'entering', stateTimer: 100, angle: 0, targetX: 0, targetY: 0, flashTimer: 0};
 ...
 // --- NEW: THE LOOT PINATA (Bigger & Slower) ---
    ...
    if (type === 'pinata') return { x, y, radius: 22, speed: 4.0 * diffMultiplier, color: '#ffd700', health: 10, maxHealth: 10, cooldown: 0, fireRate: 9999, ammo: 0, maxAmmo: 0, type: 'pinata', flashTimer: 0, overclockTimer: 0, pierceTimer: 0, escapeTimer: 600, angle: 0 };
}
function startWave() {
    ...
    if (currentWave % 5 === 0) {
        ...
        enemies = [{
            ...
            speed: 1.5, 
            color: '#ff003c', 
            health: 150 + (currentWave * 10), // Scales with wave
            maxHealth: 150 + (currentWave * 10), 
            cooldown: 0, 
...
canvas.addEventListener('touchstart', (e) => {
    ...
    for (let t of e.changedTouches) {
        let side = t.clientX < canvas.width / 2 ? 'left' : 'right';
        let j = joysticks[side];
        if (!j.active) {
            j.active = true; j.id = t.identifier; j.startX = t.clientX; j.startY = t.clientY;
            let el = document.getElementById(side + '-joystick');
            el.style.left = j.startX + 'px'; el.style.top = j.startY + 'px'; el.style.opacity = '1';
        }
    }
}, { passive: false });
...
canvas.addEventListener('touchmove', (e) => {
    for (let t of e.changedTouches) {
        for (let s in joysticks) {
            let j = joysticks[s];
            if (j.active && t.identifier === j.id) {
                let dx = t.clientX - j.startX; let dy = t.clientY - j.startY;
                let dist = Math.hypot(dx, dy); let max = 50;
                if (dist > max) { dx *= max/dist; dy *= max/dist; }
                j.vX = dx/max; j.vY = dy/max;
                let nub = document.getElementById(s + '-joystick').querySelector('.joystick-nub');
...
canvas.addEventListener('touchend', (e) => {
    for (let t of e.changedTouches) {
        for (let s in joysticks) {
            if (joysticks[s].id === t.identifier) {
                joysticks[s].active = false; joysticks[s].vX = 0; joysticks[s].vY = 0;
                document.getElementById(s + '-joystick').style.opacity = '0';
            }
        }
    }
});
...
window.addEventListener('mousedown', () => {
    // NEW: If the game is over, clicking anywhere resets it!
    if (gameOver) {
        resetGame();
        return; 
    }
...
function resetGame() {
    ...
    currentWave = 1;
    frameCount = 0;
    waveTransitionTimer = 0; // Add this line in the Reset Stats section!
...
function update() {
   ...
   // --- NEW: WAVE TRANSITION LOGIC ---
    // If all enemies are dead and we aren't already waiting...
    if (enemies.length === 0 && waveTransitionTimer <= 0 && gameStarted && !gameOver) {
        waveTransitionTimer = 180; // 180 frames = 3 seconds of breathing room
        currentWave++; 
    }
    ...
    if (waveTransitionTimer > 0) {
        waveTransitionTimer--;
        if (waveTransitionTimer <= 0) {
            startWave(); // Actually spawn the enemies when the timer hits 0
        }
    }
...
if (en.type === 'boss') {
            en.angle += 0.04; // NERF: Spins slightly slower
            en.stateTimer--;

            if (en.state === 'entering') {
                en.y += 2;
                if (en.y > 150) { en.state = 'bullet_hell'; en.stateTimer = 180; }
            } 
            else if (en.state === 'bullet_hell') {
                if (dist > 5) { en.x += (exDx / dist) * (en.speed * 0.8); en.y += (exDy / dist) * (en.speed * 0.8); }
                
                en.cooldown--;
                if (en.cooldown <= 0) {
                    for(let i=0; i<4; i++) {
                        let offset = (Math.PI / 2) * i;
                        // NERF: Now shoots slower 'standard' bullets instead of fast 'sniper' bullets
                        shootBullet(en.x, en.y, en.x + Math.cos(en.angle + offset)*100, en.y + Math.sin(en.angle + offset)*100, false, 'standard');
                    }
                    en.cooldown = 18; // NERF: Much slower fire rate (was 8, now 18)
                }
                if (en.stateTimer <= 0) { 
                    en.state = 'dashing'; en.stateTimer = 70; en.targetX = player.x; en.targetY = player.y; 
                    spawnFloatingText(en.x, en.y - 60, "CHARGE!", "#ffcc00", 25);
                }
            } 
           else if (en.state === 'dashing') {
                let chargeDx = en.targetX - en.x; let chargeDy = en.targetY - en.y; let chargeDist = Math.hypot(chargeDx, chargeDy);
                
                if (chargeDist > 10) { 
                    en.x += (chargeDx / chargeDist) * 7; 
                    en.y += (chargeDy / chargeDist) * 7; 
                } else {
                    // NEW: When it finishes the dash, it enters 'vulnerable' mode
                    en.state = 'vulnerable'; 
                    en.stateTimer = 120; // Sits still for 2 seconds
                    spawnFloatingText(en.x, en.y - 60, "VULNERABLE!", "#00f3ff", 25);
                }
                
                // (Keep your collision logic here)
            }
            else if (en.state === 'vulnerable') {
                // Boss does nothing, takes 2x damage (handled in bullet hit logic)
                if (en.stateTimer <= 0) { en.state = 'bullet_hell'; en.stateTimer = 180; }
            }
        }
            ...
            if (en.cooldown <= 0 && en.ammo > 0) {
                shootBullet(en.x, en.y, player.x, player.y, false, en.type); 
                if (en.pierceTimer > 0) {
                    let b = bullets[bullets.length - 1];
                    b.piercing = true; b.maxBounces = 0;
                }
                en.ammo--; en.cooldown = en.fireRate; 
            }
        }
            ...
            for (let e = enemies.length - 1; e >= 0; e--) {
               ...
               // --- 15% RANDOM CRITICAL HIT CHANCE ---
                  ...
                  if (en.health <= 0) {
                       ...
                       if (en.type === 'boss') {
                            // ... [Your existing Boss death logic stays exactly the same here] ...
                            pointsGained = 5000;
                            triggerHitStop(30); 
                            triggerShake(50); 
