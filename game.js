...
let currentWeapon = 'basic'; // 'basic', 'shredder', 'railgun'
let weaponTimer = 0; // How long the power-up lasts (in frames)
let railgunBeams = []; // Array to hold the instantaneous laser beams
// This will replace ammoDrops
let blackHoles = [];
let sweepingLasers = [];
let hazardDamageCooldown = 0; // Prevents the laser/blackhole from killing you instantly
...
function getSafeSpawn(radius) {
    let x, y, dist;
    let attempts = 0; // NEW: Track attempts
    do {
        x = Math.random() * canvas.width;
        y = Math.random() * canvas.height;
        let dx = x - player.x;
        ...
        attempts++;
    } while (dist < 300 && attempts < 100); // NEW: Give up after 100 tries
...
function startWave() {
   ...
   // Inside startWave()...
    ...
    // Every 3 waves, spawn a massive Gravity Well (Black Hole)
    if (currentWave >= 3 && currentWave % 3 === 0) {
        blackHoles.push({
            x: canvas.width / 2 + (Math.random() - 0.5) * 400,
            y: canvas.height / 2 + (Math.random() - 0.5) * 400,
            radius: 40,
            power: 12 // Gravity pull strength
        });
    }

    // Every 4 waves, call in an Orbital Sweeping Laser
    if (currentWave >= 4 && currentWave % 4 === 0) {
        sweepingLasers.push({
            y: 0,
            height: 80,
            vy: 2.5, // Speed of the sweep moving up and down
            state: 'warning', 
            timer: 120 // 2 seconds of warning telegraph
        });
    }
    }
...
function shoot(targetX, targetY) {
    // --- 🚨 BUG 1 FIX: Prevent firing 60 times a second! ---
    if (autoFireTimer > 0) return; 

    // Set custom fire-rates based on the heavy weapons
    if (currentWeapon === 'shredder') {
        autoFireTimer = 35; // Shotgun shoots slower
    } else if (currentWeapon === 'railgun') {
        autoFireTimer = 50; // Railgun has a massive recharge time
    } else {
        autoFireTimer = 12; // Normal fast blaster
    }

    // Fallback to mouse coordinates if target is undefined
    let tx = targetX !== undefined ? targetX : mouse.x;
    let ty = targetY !== undefined ? targetY : mouse.y;
    ...
    if (currentWeapon === 'basic') {
    ...
    } else if (currentWeapon === 'shredder') {
        const spreadAngles = [-0.4, -0.2, 0, 0.2, 0.4]; 
        spreadAngles.forEach(offset => {
            bullets.push({
                x: player.x,
                y: player.y,
                vx: Math.cos(angle + offset) * (speed * 1.2),
                vy: Math.sin(angle + offset) * (speed * 1.2),
                radius: 4,
                color: '#ff8c00', 
                damage: 0.8, 
                piercing: false
        ...
        enemies.forEach(en => {
            ...
            let t = (dx * Math.cos(angle) + dy * Math.sin(angle));
            
            if (t > 0) { 
                let projX = player.x + t * Math.cos(angle);
                let projY = player.y + t * Math.sin(angle);
                let distanceToLine = Math.hypot(en.x - projX, en.y - projY);
                
                if (distanceToLine < en.radius + 15) {
                    en.hp -= 5; 
                }
            }
        });
...
function spawnDrop(x, y) {
    ...
    let type = 'credit'; 
    let color = '#ffd700'; // Gold for credits

    if (rand < 0.1) {
        // 10% chance for a Shredder (Shotgun)
        type = 'shredder';
        color = '#ff8c00'; // Orange
    } else if (rand < 0.15) {
        // 5% chance for a Railgun
        type = 'railgun';
...
function update() {
    ...
    if (weaponTimer > 0) {
        weaponTimer--;
        if (weaponTimer <= 0) {
            currentWeapon = 'basic'; // Revert back to normal
            spawnFloatingText(player.x, player.y - 20, "WEAPON DEPLETED", "#ff0000", 14);
        }
    }
    
    // Update railgun beams
    railgunBeams.forEach(beam => beam.life--);
    railgunBeams = railgunBeams.filter(beam => beam.life > 0);
   ...
   // 3. OVERCLOCK TIMER (Countdown only)
    ...
    blackHoles.forEach(bh => {
        ...
        // Dark Event Horizon
        ctx.beginPath();
        ctx.arc(0, 0, bh.radius * (1 + pulse * 0.1), 0, Math.PI * 2);
        ctx.fillStyle = '#000000';
        ctx.shadowBlur = 40 + pulse * 20;
        ctx.shadowColor = '#8a2be2'; // Deep purple neon glow
        ctx.fill();
        
        // Swirling Accretion Disk
        ctx.rotate(Date.now() / 300);
        ctx.beginPath();
        ctx.ellipse(0, 0, bh.radius * 2.5, bh.radius * 0.4, 0, 0, Math.PI * 2);
    ...
    sweepingLasers.forEach(laser => {
        ...
        if (laser.state === 'warning') {
            ...
            if (Math.floor(Date.now() / 150) % 2 === 0) {
                ...
                ctx.fillText('! ORBITAL LASER LOCKING ON !', canvas.width/2, laser.y + laser.height/2 + 8);
            }
        } else if (laser.state === 'active') {
            // Active Burning Beam
            let flicker = Math.random() * 0.5;
            ctx.fillStyle = `rgba(255, ${Math.random()*100}, 0, ${0.8 + flicker})`;
            ctx.shadowBlur = 30;
            ...
            ctx.fillRect(0, laser.y + laser.height/3, canvas.width, laser.height/3);
        }
        ctx.restore();
    });
...
// --- CONSOLIDATED DROP COLLECTION (Cubes, Credits, & Weapons) ---
    for (let i = drops.length - 1; i >= 0; i--) {
        let d = drops[i];
        d.life--; // All drops lose life over time
        
        let dx = player.x - d.x; 
        let dy = player.y - d.y;
        let dist = Math.hypot(dx, dy);
        
        // 1. MAGNETIC VACUUM EFFECT
        // We now pull ALL drop types if the player is close enough
        let vacRadius = (player.upgrades && player.upgrades.magneticHull) ? 450 : 150;
        if (dist > 0 && dist < vacRadius) { 
            d.x += (dx / dist) * 12; 
            d.y += (dy / dist) * 12; 
        }
        ...
        if (dist < player.radius + 15) {
            ...
            if (d.type === 'cube') {
                ...
                score += 50;
                metaCredits += 1;
                spawnFloatingText(d.x, d.y, "+1 CR", "#ffd700", 14);

            } else if (d.type === 'shredder') {
                // --- WEAPON: SHREDDER ---
                currentWeapon = 'shredder';
                weaponTimer = 600; // 10 seconds
                spawnFloatingText(d.x, d.y, "SHREDDER ACTIVE!", "#ff8c00", 20);

            } else if (d.type === 'railgun') {
                // --- WEAPON: RAILGUN ---
                currentWeapon = 'railgun';
                weaponTimer = 420; // 7 seconds
                spawnFloatingText(d.x, d.y, "RAILGUN ACTIVE!", "#ff00ff", 20);
            }

            drops.splice(i, 1); // Remove item after collection
            continue;
        }
...
// Drops Ticking & Collision
    ...
    // ... all your other update code (bullets, enemies, etc) ...

    // ==========================================
    // --- STAGE HAZARDS PHYSICS ---
    // ==========================================
    if (hazardDamageCooldown > 0) hazardDamageCooldown--;

    // 1. BLACK HOLE GRAVITY PULL
    blackHoles.forEach(bh => {
        // Pull the Player
        let pdx = bh.x - player.x; let pdy = bh.y - player.y;
        let pDist = Math.sqrt(pdx * pdx + pdy * pdy);
        if (pDist > 5 && pDist < bh.radius * 10) {
            let force = bh.power / (pDist * 0.1);
            player.x += (pdx / pDist) * force;
            player.y += (pdy / pDist) * force;
        }
        
        // Pull Enemies
        enemies.forEach(en => {
            let edx = bh.x - en.x; let edy = bh.y - en.y;
            let eDist = Math.sqrt(edx * edx + edy * edy);
            if (eDist > 5 && eDist < bh.radius * 10) {
                let force = (bh.power * 0.8) / (eDist * 0.1);
                en.x += (edx / eDist) * force;
                en.y += (edy / eDist) * force;
            }
        });

        // If player touches the absolute center (Event Horizon)
        if (pDist < bh.radius * 0.5 && hazardDamageCooldown <= 0) {
            takeDamage();
            hazardDamageCooldown = 60; // 1 second immunity to prevent instant death
        }
    });

    // 2. ORBITAL SWEEPING LASER
    sweepingLasers.forEach(laser => {
        if (laser.state === 'warning') {
            laser.timer--;
            if (laser.timer <= 0) {
                laser.state = 'active';
                laser.timer = 400; // Beam lasts for several seconds
                if (typeof triggerShake === 'function') triggerShake(15); 
            }
        } else if (laser.state === 'active') {
            laser.y += laser.vy; // Sweep across the screen
            
            // Bounce off top and bottom of screen
            if (laser.y > canvas.height - laser.height || laser.y < 0) {
                laser.vy *= -1; 
            }
            
            // Player Collision (Burn the player!)
            if (player.y > laser.y && player.y < laser.y + laser.height) {
                // If player doesn't have invincibility frames from dashing
                if (hazardDamageCooldown <= 0 && !player.isDashing) { 
                    takeDamage();
                    hazardDamageCooldown = 60; 
                }
            }
            
            // Enemy Collision (The laser burns enemies too!)
            enemies.forEach(en => {
                if (en.y > laser.y && en.y < laser.y + laser.height) {
                    en.hp -= 0.1; // Rapidly melt enemy health
                }
            });

            laser.timer--;
            if (laser.timer <= 0) laser.state = 'done';
        }
    });
    
    // Clean up finished lasers
    sweepingLasers = sweepingLasers.filter(l => l.state !== 'done');

} // <--- THIS IS THE CLOSING BRACKET FOR function update()
...
function draw() {
    ...
    railgunBeams.forEach(beam => {
        ...
        ctx.strokeStyle = `rgba(255, 0, 255, ${beam.life / 15})`;
        ctx.lineWidth = 10 * (beam.life / 15); // Gets thinner as it fades
        ctx.shadowBlur = 20;
